...
bool doSideBar(uint16_t cmd, int16_t enc, int16_t enca)
{
  ...
  switch(cmd)
  {
    ...
    case CMD_WEBSDR:     webSdrHandleEncoder(enc);break;
    case CMD_WIFISETUP:  wifiSetupHandleEncoder(enc);break;
...
bool clickHandler(uint16_t cmd, bool shortPress)
{
  switch(cmd)
  {
    ...
    case CMD_WEBSDR:
      ...
      break;
    case CMD_WIFISETUP:
      wifiSetupHandleClick();
      if (wifiSetupWantsExit())
      {
        wifiSetupExit();
        currentCmd = CMD_NONE;
      }
...
void drawSideBar(uint16_t cmd, int x, int y, int sx)
{
  ...
  switch(cmd)
  {
    ...
    case CMD_WEBSDR:     break;  // WebSDR has its own full-screen UI
    case CMD_WIFISETUP:  break;  // WiFi Setup has its own full-screen UI
