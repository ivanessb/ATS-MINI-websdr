...
// Microstepping state
static int32_t  microStepRemaining = 0;    // SSB (Hz)
static int32_t  amFmMicroStepRemaining = 0; // AM/FM (kHz or FM units)
...
void setup()
{
  ...
  // Initialize LVGL UI (currently hidden)
  //lvglInit();

  // PWM audio output on IO11 is now managed by PwmAudio module
  // audioPwmInit();

  // Initialize WebSDR client (does not connect yet)
  webSdrInit();
...
//
// Process microstepping: apply pending tuning changes in small increments
//
bool processMicrostepping()
{
  bool needRedraw = false;
  bool idle = (millis() - lastTuneTime) > 80;

  // SSB microstepping (Hz via BFO)
  if(isSSB() && microStepRemaining != 0)
  {
    int32_t step;
    int32_t microStepSize = max((int32_t)10, (int32_t)(getCurrentStep()->step / 10));
    int32_t fullStep = (int32_t)getCurrentStep()->step;

    if(idle || abs(microStepRemaining) <= microStepSize)
    {
      step = microStepRemaining;
    }
    else if(abs(microStepRemaining) > fullStep)
    {
      step = microStepRemaining / 2;
    }
    else
    {
      step = (microStepRemaining > 0) ? microStepSize : -microStepSize;
    }

    updateBFO(currentBFO + step, true);
    microStepRemaining -= step;
    needRedraw = true;
  }
  else
  {
  ...
  // AM/FM microstepping (kHz / FM units)
  if(!isSSB() && amFmMicroStepRemaining != 0)
  {
    int32_t step;
    int32_t minStep = (currentMode == FM) ? 1 : 1; // 1 unit = 10kHz FM, 1kHz AM
    int32_t microStepSize = max(minStep, (int32_t)(getCurrentStep()->step / 10));
    int32_t fullStep = (int32_t)getCurrentStep()->step;

    if(idle || abs(amFmMicroStepRemaining) <= microStepSize)
    {
      step = amFmMicroStepRemaining;
    }
    else if(abs(amFmMicroStepRemaining) > fullStep)
    {
      step = amFmMicroStepRemaining / 2;
    }
    else
    {
      step = (amFmMicroStepRemaining > 0) ? microStepSize : -microStepSize;
    }

    updateFrequency(currentFrequency + step, true);
    amFmMicroStepRemaining -= step;
    needRedraw = true;
  }
  else
  {
...
void loop()
{
    ...
    if(encCount)
    {
      switch(currentCmd)
      {
        ...
        case CMD_SEEK:
          ...
          break;
        case CMD_WEBSDR:
          // WebSDR menu navigation / value editing
          webSdrHandleEncoder(encCountAccel);
          needRedraw = true;
          break;
        case CMD_WIFISETUP:
          // WiFi Setup navigation
          wifiSetupHandleEncoder(encCountAccel);
          needRedraw = true;
          break;
        default:
  ...
  if((currentTime - elapsedCommand) > ELAPSED_COMMAND)
  {
    ...
    if(currentCmd != CMD_NONE && currentCmd != CMD_SEEK && currentCmd != CMD_SCAN && currentCmd != CMD_MEMORY && currentCmd != CMD_WEBSDR && currentCmd != CMD_WIFISETUP)
    {
      currentCmd = CMD_NONE;
      needRedraw = true;
    }
  ...
  {
    if(!webSdrIsActive() && !wifiSetupIsActive())
  ...
  // Process SSB microstepping
  needRedraw |= processMicrostepping();

  // WebSDR mode: run task and draw custom screen
  if(webSdrIsActive())
  {
    webSdrTask();
    // Redraw on user interaction or periodically (throttled to reduce battery flicker)
    if(needRedraw || (currentTime - background_timer > 200))
    {
      spr.fillSprite(TH.bg);
      drawWebSdrScreen();
      spr.pushSprite(0, 0);
  ...
  // WiFi Setup mode: full-screen WiFi management
  else if(wifiSetupIsActive())
  {
    if(needRedraw || (currentTime - background_timer > 200))
    {
      spr.fillSprite(TH.bg);
  ...
  {
    // Use original UI (LVGL UI hidden)
    if(needRedraw) drawScreen();
  }

  // Audio test tones disabled — PwmAudio module handles IO11 for WebSDR
  // static bool audioTestDone = false;
  ...
  //   audioPwmTestPlay();
  // }
  // if(audioMuted && !audioPwmTestRunning()) {
