...
## WebSDR PWM audio hardware requirement

When using WebSDR mode, audio is generated by ESP32 PWM output.

IMPORTANT: Do not connect PWM output directly to the speaker amplifier IC input.
You must insert a low-pass filter between PWM output and amplifier input.

Recommended starting point:

* Series resistor: 2.2k
* Shunt capacitor to AGND: 100nF
* AC coupling capacitor after filter: 1uF

You can fine tune values based on your PCB revision and desired audio response.

Reference schematic for the WebSDR PWM path:

![](_static/websdr-pwm-lpf-schematic.png)
