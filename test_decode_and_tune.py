"""Test: full decode in on_message + tunes from another thread. Does THIS disconnect?"""
import sys
import struct
import threading
import time
...
class TestPlayer:
    def __init__(self):
        ...
        self.msg_count = 0; self.total_samples = 0; self.smeter = 0
...
if __name__ == "__main__":
    ...
    # Send tune commands from THIS thread (simulating user tuning)
    ...
    for i in range(100):
        ...
        try:
            ...
            with player.pcm_lock:
                player.pcm_buf.clear()
            if tune_count <= 3 or tune_count % 10 == 0:
    ...
    if closed.is_set():
        print(f"  DISCONNECTED after {elapsed:.1f}s, {tune_count} tunes")
    else:
        print(f"  SURVIVED {elapsed:.1f}s with {tune_count} tunes")
        ws.close()
